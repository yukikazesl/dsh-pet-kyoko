...
export const zh = {
  ...
  typingEnabled: '打字互动',
  typingEnabledHint: '启用后检测到系统按键活动时播放打字动画（类似直播伴侣；桌面全局有效）。',
  displayLabel: '显示位置',
...
export const en = {
  ...
  typingEnabledHint:
    'When enabled, system key activity plays the typing animation (live-companion style; works globally on desktop).',
    ...
    const addPet = () => {
      ...
      setPets((list) => [
        ...list,
        {
          ...
          typingEnabled: !!tpl.typingEnabled,
          display: tpl.display,
              ...
              children: [
                ...
                h('label', {
                  ...
                  children: [
                    ...
                    h('input', {
                      type: 'checkbox',
                      checked: !!cur.typingEnabled,
                      disabled: busy,
                      onChange: (e: ChangeEvent<HTMLInputElement>) => updateSel({ typingEnabled: e.target.checked }),
                      style: { width: '16px', height: '16px', accentColor: 'var(--dsw-alias-state-business-primary)' },
                    }),
                    h('span', {
                      style: { fontSize: '11px', color: 'var(--dsw-alias-label-tertiary)' },
                      children: t('typingEnabledHint'),
                    }),
                  ],
                }),
                h('label', {
